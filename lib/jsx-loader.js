...
function selectFileName(_path) {
    if (typeof (_path) !== 'string')
        throw ('the path is not string');
    var _a = path.parse(_path), name = _a.name, dir = _a.dir;
    if (name === 'index') {
...
module.exports = function (source) {
    ...
    if (!canTraverse)
        return source;
    var preName = selectFileName(resourcePath);
    _cache = {};
    var classHashChange = {};
    var options = {
        JSXAttribute: function (path) {
            ...
            classNames.split(" ").map(function (className) {
                ...
                var newClassName = preName + "_" + className + "_" + createHash(resourcePath + className);
